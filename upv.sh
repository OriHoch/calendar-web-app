...
docker_build_upv() {
    ...
    local DOCKER_TAG="upv-`uuidgen`"
    if [ -f "${UPV_DOCKER_PATH}/upv.Dockerfile" ]; then
        local DOCKER_FILE="upv.Dockerfile"
    else
        local DOCKER_FILE="Dockerfile"
    fi
    local CMD="docker build -t ${DOCKER_TAG} -f ${DOCKER_FILE} ${UPV_DOCKER_PATH}"
    debug $CMD >/dev/stderr
    if [ "${UPV_DEBUG}" == "1" ]; then
        $CMD > /dev/stderr
    else
        $CMD > /dev/null
...
then
    ...
    if [ "${UPV_DOCKER_PATH}" == "" ]; then
        ...
        DOCKER_IMAGE=`docker_build_upv "${UPV_DOCKER_PATH}"`
